...
const Form = ({ formData, locale }: FormProps) => {
    ...
    const [formValues, handleImputChange, resetInputs] = useForm(initialState);
    ...
    return (
        <form
            ...
            className="z-10	grid gap-y-12 gap-x-8 grid-cols-1 md:grid-cols-2 px-4 md:mx-12 lg:mx-20"
        >
            <input
                type="text"
...

            {
                //Response message
                resMessage && (
                    <p className="text-lg md:text-xl text-white col-start-1 col-end-2 md:col-end-3">
                        {resMessage}
                    </p>
                )
            }
            ...
            <button
                disabled={sending}
                className="text-center inline-block  w-full md:w-1/2 font-medium  text-base md:text-lg lg:text-xl  hover:text-white bg-black	hover:bg-primary linear duration-300 rounded text-white py-4 px-6 uppercase cursor-pointer	
            ...
            >
                {!sending ? (
                    ...
                    <svg
                        className="inline mr-2 w-8 h-8 text-white animate-spin dark:text-gray-600 fill-primary"
                        viewBox="0 0 100 101"
                        fill="none"
